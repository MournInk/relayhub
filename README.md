...
当前仓库已经包含可运行的 Go 后端、可静态导出的 Next.js + Radix UI + Tailwind 控制台、Tauri 桌面壳、Docker 构建与 GitHub Actions。
...
- 前端：Next.js App Router + Radix UI + Tailwind CSS
- 桌面：Tauri 2，目标平台为 macOS / Windows / Linux
- 后端：Go
- 存储：SQLite（本地优先）
- 会话与治理状态：当前实现以内存和本地数据库为主
...
- 桌面一体版：
  - 安装 GitHub Actions `desktop-tauri` workflow 产出的桌面包后，Tauri 会自动拉起内置的 Go sidecar。
  - 控制台静态资源已经随 App 打包，首次启动默认连接 `http://127.0.0.1:4317`。
  - 配置文件默认落在系统应用配置目录，数据库默认落在系统应用数据目录。
- 独立后端 + 浏览器控制台：
  - 后端默认监听 `:8080`。
  - 浏览器打开控制台时，默认连接 `http://127.0.0.1:8080`。
...
`desktop-tauri` workflow 现在会为 Linux / Windows / macOS 构建一体化桌面包，并上传到该次 Actions run 的 artifact：

- `relayhub-desktop-linux-x64`
- `relayhub-desktop-windows-x64`
- `relayhub-desktop-macos-arm64`
